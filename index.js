...
fs.readdirSync("./source/Commands", { encoding: "utf8" })
  .filter(file => file.endsWith(".js"))
  .forEach(files => {
    let command = require(`./source/Commands/${files}`);
    if (!command.name)
      return console.log(`Hatalı Kod Dosyası => [./source/Commands/${files}]`);
...
client.on("ready", async () => {
  ...
  client.user.setPresence({
    activity: { name: "Lucy was here", type: "WATCHING" },
    status: "dnd"
  });
...
client.on("guildMemberAdd", async member => {
  let kontrol =
    Date.now() - member.user.createdTimestamp > 1000 * 60 * 60 * 24 * 15
      ? "güvenli"
      : "şüpheli";
...
Sunucumuza hoşgeldin ${member}! seninle birlikte **${
      member.guild.memberCount
    }** kişiye ulaştık!

Sol taraftaki **V.valueirmed** odalarına giriş yapıp kaydını yaptırabilirsin, <@&${
      value.Register.registerStaff[0]
    }> rolüne sahip yetkililerimiz seninle ilgilenecektir. 

Sunucu kurallarımız <#${
      value.Register.Kurallar
    }> kanalında belirtilmiştir. Unutma kayıt olduğunda kuralları okumuş sayılırsın ve buna göre ceza-i işlemler uygulanır !

Hesabın **${moment(member.user.createdAt)
      .locale("tr")
      .format("LLL")}** tarihinde \`(${client.tarihHesapla(
      member.user.createdAt
    )})\`  açılmış yani **${kontrol}**. Tagımızı (\`${
      value.Register.Tag
    }\`) alarak bizlere destek olabilirsin!`);
    ...
    await client.channels.cache.get(value.Register.KayıtKanal).send(
      new Discord.MessageEmbed()
        ...
        .setFooter(value.Embed.Footer)
        .setTimestamp().setDescription(`
${member} sunucuya katıldı fakat hesabı **${moment(member.user.createdAt)
        .locale("tr")
        .format("LLL")}** tarihinde \`(${client.tarihHesapla(
        member.user.createdAt
      )})\` açıldığı için <@&${value.Register.Şüpheli}> rolü verildi.`)
    );
...
client.on("guildMemberRemove", async member => {
  ...
  if (sonisim)
    patavatsizDatabase.isimGuncelle(member, sonisim.Name, "Sunucudan Ayrılma");
});
